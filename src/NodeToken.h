...
Stack<string> _in_for;
string extvariablestore;
int nbofextern = 0;
...
void _visitstoreLocalVariableNodeAsRegister(NodeToken *nd) {
  if (nd->getVarType()->_varType == __float__) {
    bufferText->addAfter(
        bufferText->sp.pop(),
        string_format("rfr a%d,f%d", nd->target, register_numl.get()));
  } else
    bufferText->addAfter(
        bufferText->sp.pop(),
...
void _visitlocalVariableNodeAsRegister(NodeToken *nd) {
  if (nd->getVarType()->_varType == __float__)
    bufferText->addAfter(
        string_format("wfr f%d,a%d", register_numl.get(), nd->target));
  else
    bufferText->addAfter(
...
void _visitnumberNode(NodeToken *nd) {
  ...
  if (nd->children_size() > 0) {
    ...
    if (nd->getVarType()->_varType == __float__) {
      ...
      header.addAfter(val);
      // point_regnum++;
      bufferText->addAfter(string_format("l32r a%d,@_%s_%d", 9, "local_var",
                                         local_var_num)); // YBA 25-02-2025
      bufferText->addAfter(string_format("lsi f%d,a%d,0", register_numl.get(),
                                         9)); // YBA 25-02-2025
      bufferText->sp.push(bufferText->get());
      // point_regnum--;
      local_var_num++;
      ...
      if (__num >= 2048) {
        ...
        bufferText->addAfter(string_format(
            "l32i a%d,a%d,0", register_numl.get(), 9)); // YBA 25-02-2025
        bufferText->sp.push(bufferText->get());
        // point_regnum--;
        local_var_num++;
        register_numl.decrease();
      ...
      if (__num >= 2048 or __num <= -2047) {
        ...
        // point_regnum++;
        bufferText->addAfter(string_format("l32r a%d,@_%s_%d", 9, "local_var",
                                           local_var_num)); // YBA 25-02-1975
        bufferText->addAfter(string_format(
            "l32i a%d,a%d,0", register_numl.get(), 9)); // YBA 25-02-1975
        bufferText->sp.push(bufferText->get());
        // point_regnum--;
        local_var_num++;
        register_numl.decrease();
  ...
  if (nd->getChildAtPos(1)->_vartype == __float__) {
    ...
    if (nd->_total_size > 127) // 127
    {
      ...
      if (regular) {
        ...
        bufferText->addAfter(string_format("%s%s a%d,%d,%s_if", compop.c_str(),
                                           _add.c_str(), leftl, f,
                                           nd->getTargetText()));
      }
      bufferText->addAfter(string_format("j %s_end", nd->getTargetText()));
      ...
      if (regular)
        bufferText->addAfter(string_format("%s%s a%d,a%d,%s_end",
                                           compop.c_str(), _add.c_str(), numl,
                                           leftl, nd->getTargetText()));
      else {
        bufferText->blankCurrent();
        f = valBranchImmediate(f);
        bufferText->addAfter(string_format("%s%s a%d,%d,%s_end", compop.c_str(),
                                           _add.c_str(), leftl, f,
                                           nd->getTargetText()));
      }
      // bufferText->addAfter(_compare.back()+1,string_format("j %s_end",
...
void _visitforNode(NodeToken *nd) {
  ...
  nd->getChildAtPos(0)->visitNode();
  register_numl.pop();
  if (nd->getChildAtPos(nd->children_size() - 1)->_nodetype == onlyNode) {
    bufferText->addAfter(string_format(
        "movExt a7,@_ext_%s",
        (nd->getChildAtPos(nd->children_size() - 1)->getTargetText())));
    boolextern = true;
  }
  ...
  if (nd->children_size() > 4) {
    if (nd->getChildAtPos(4)->_nodetype != onlyNode) {
      nd->getChildAtPos(4)->visitNode();
    }
  }
  if (nd->children_size() > 5) {

    nd->getChildAtPos(5)->visitNode();
  }
  register_numl.pop();
  ...
  case TokenLessThan:
    ...
    nd->getChildAtPos(1)->getChildAtPos(0)->type =
        TokenMoreOrEqualThan; // greater or equal
    //  bufferText->addAfter( string_format("%s_end:\n",nd->target.c_str()));
    break;
  case TokenDoubleEqual:
    ...
    if (v->size > 1) {
      for (int i = 0; i < v->size; i++) {
        ...
        if (i > -1)
          ...
          bufferText->addAfter(
              string_format("mov a%d,a10", register_numl.get()));
        // register_numl--;
        start += v->sizes[i];
        bufferText->sp.push(bufferText->get());
      }
    ...
    for (int i = 0; i < nd->children_size(); i++) {
      ...
      if (start >= _STACK_SIZE) {
        ...
        if (reg_num <= 7) {
          ...
          if (nd->getChildAtPos(i)->isPointer) {
            bufferText->addAfter(
                string_format("s32i a%d,a1,%d", reg_num, start));
          } else if (nd->getChildAtPos(i)->getVarType()->_varType ==
                     __float__) {
            bufferText->addAfter(
                string_format("s32i a%d,a1,%d", reg_num, start));
          ...
          asmInstruction asmInstr = nd->getChildAtPos(i)->getVarType()->load[0];
          bufferText->addAfter(string_format(
              "%s a15,a1,%d", asmInstructionsName[asmInstr].c_str(),
...
*/
        ...
        if (par < nd->children_size() - 1) {
          ...
          for (int h = 1; h < nd->children_size() - par; h++) {
            ...
            if (nb > 2)
              ...
              bufferText->addAfter(
                  string_format("mull a10,a%d,a11", register_numl.get()));
          }
          if (nb > 2)
            bufferText->addAfter(
                string_format("add a10,a10,a%d", register_numl.get()));
          ...
          // bufferText->addAfter(string_format("mov a10,a%d",
          // register_numl.get()));
          bufferText->addAfter(string_format(
              "add a%d,a10,a%d", register_numl.get(), register_numl.get()));
          // else
          // bufferText->addAfter(string_format("add a%d,a%d,a%d",
          // register_numl.get(),register_numl.get(), register_numl.get()));
          //         if(nb<=2)
          // bufferText->addAfter("nop");
        }
      }
  ...
  if (!boolextern)
    ...
    bufferText->addAfter(string_format("mov a%d,a7", point_regnum));
  if (nd->isPointer && nd->children_size() > 0) {
    // f=f+number.f;
    for (int i = 0; i < v->total_size; i++) {
      bufferText->addAfter(string_format("add a%d,a%d,a%d", point_regnum,
...
void _visitbreakNode(NodeToken *nd) {
  ...
  bufferText->addAfter(string_format("j %s_break", nd->getTargetText()));
}
void _visitUnknownNode(NodeToken *nd) {}
          ...
          if (d[0].compare("bnez") == 0 or d[0].compare("beqz") == 0) {
            ...
            */
            // printf("new %s\n",newstr.c_str());
                     ...
                     d[0].compare("beq") == 0 or d[0].compare("bne") == 0 or
                     d[0].compare("blti") == 0 or d[0].compare("bgei") == 0 or
                     d[0].compare("beqi") == 0 or d[0].compare("bnei") == 0) {
