...
result_parse_line parseline(line sp, parsedLines *asm_parsed) {
  ...
  if (sp.opcde.compare("bgei") == 0) {
    ...
    result_parse_line ps =
        parseOperandes(sp.operandes, 3, op_blti, 3, bin_bgei);
    ps.op = opCodeType::jump;
    ps.calculateOfssetJump = jump_blt;
  ...
  if (sp.opcde.compare("bnei") == 0) {
    ...
    result_parse_line ps =
        parseOperandes(sp.operandes, 3, op_blti, 3, bin_bnei);
    ps.op = opCodeType::jump;
    ps.calculateOfssetJump = jump_blt;
  ...
  if (sp.opcde.compare("beqi") == 0) {
    ...
    result_parse_line ps =
        parseOperandes(sp.operandes, 3, op_blti, 3, bin_beqi);
    ps.op = opCodeType::jump;
    ps.calculateOfssetJump = jump_blt;
...
uint8_t *createBinaryHeader(parsedLines *asm_parsed) {
  ...
  for (int js = 0; js < jsonVar.size(); js++) {
    ...
    binary_header = binary_header + text_size;
    int index = findLabel(
        string_format("%s%s", "@_", jsonVar[js].variable.c_str()), asm_parsed);
    // printf("ibndexw:%d %s\n",index,string_format("%s%s
    // %s","@_",jsonVar[js].variable.c_str(),jsonVar[js].json.c_str()).c_str());
