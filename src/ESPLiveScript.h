...
public:
  ...
  void parseStatement() {
    ...
    if (Match(TokenIdentifier) &&
      ...
      if (_for_depth_reg.get() <= _MAX_FOR_DEPTH_REG_2 and
          (d == __float__ or d == __int__ or d == __s_int__ or
           d == __uint8_t__ or d == __uint32_t__ or d == __uint16_t__)) {
        _is_variable_as_register.set(true);
      }
      // else
      // _is_variable_as_register.set(false));
      ...
      //  NodeToken _nd=nodeTokenList.get();
      _is_variable_as_register.set(false);

      current_cntx->addVariable(nodeTokenList.get()); // 27/05
      if (Match(TokenComma)) {
        while (Match(TokenComma)) {
          next();
  ...
  void parseExprConditionnal() {
    ...
    while (Match(TokenDoubleEqual) || Match(TokenNotEqual) ||
           Match(TokenLessOrEqualThan) || Match(TokenLessThan) ||
           Match(TokenMoreOrEqualThan) || Match(TokenMoreThan)) {
    ...
    while (Match(TokenEndOfFile) == false) {
      ...
      if (Match(TokenKeywordStruct)) {
        ...
        if (!Match(TokenAs)) {
          Error.error = 1;
          Error.error_message =
              string_format("Missing As in Json %s", linepos().c_str());
          return;
        }
        next();
