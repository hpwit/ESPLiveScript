...
define LED_COLS 128 define LED_ROWS 96 define NUM_LEDS 12288 define
    PI 3.1415926535 define panel_width 128 define speed 1 define
        nb_branches 5 uint8_t C_X,
    C_Y, mapp;
  ...
  while (true) {
    for (uint8_t x = 0; x < LED_COLS; x++) {
      for (uint8_t y = 0; y < LED_ROWS; y++) {
        ...
        leds[y * panel_width + x] =
            hsv(t + radius, 255,
                sin8(sin8((angle * 4 - radius * mapp) / 4 + t) +
                     angle * nb_branches - 2 * t));
      }
    }
    sync();
    t = t + speed;
