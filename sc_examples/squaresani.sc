...
define width 128 define height 96 define scale 1 define speed 5 define
    nSquares 4

    define cosT 0.999998 define sinT 0.0001745 float t1;
float t2;
float timebase;
// var cosT = cos(0.1), sinT = sin(0.1);
uint32_t __baseTime[1];
__ASM__ void setTime(){"entry a1,32"
                       "rsr a14,234"
                       "l32r a5,@___baseTime"
                       "s32i a14,a5,0"
                       "retw.n"} uint32_t __deltamillis[1];
__ASM__ uint32_t millis(){"entry a1,32"
                          "l32r a5,@___baseTime"
                          "l32i a14,a5,0"
                          "rsr a13,234"
                          ...
                          //  "l32r a4,@_stackr"
                          // "s32i a13,a4,0"
                          "retw.n"} __ASM__ uint32_t elapseMillis() {
  "entry a1,32"
...
void render2D(int x, int y) {
  ...
  leds[128 * y + x] =
      hsv((t2 + x0 * sx + y0 * sy) * 255, 255, (dx * dx * dx) * 255);
}
